...
impl Node {
    ...
    where
        M: Fn(&mut ChainSpec),
    {
        let chain_spec_path = self.working_dir().join("specs/dev.toml");
    ...
    fn init(working_dir: PathBuf) -> Self {
        ...
        let mut chain_spec: ChainSpec = {
            let chain_spec_path = working_dir.join("specs/dev.toml");
    ...
    pub fn start(&mut self) {
        let mut child_process = Command::new("ckb-growth")
