...
use crate::case::rfc0029::is_rfc0029_switched;
...
pub struct RFC0029V2019;

impl Case for RFC0029V2019 {
    ...
    fn run(&self, nodes: Nodes) {
        ...
        // Make sure we are before rfc0029 switch
        ...
        for (i, (script_hash_type, str_cell_deps, expected_result)) in cases.into_iter().enumerate()
        {
            ...
            if expected_result == PASS {
                ...
                // We test both node2019 and node2021 before rfc0029 switching, they are v2019
                node2021
                ...
                // We test both node2019 and node2021 before rfc0029 switching, they are v2019
                node2019.pull_node(node2021).expect(&format!(
                    "{} pull from {}",
                    node2019.node_name(),
                    node2021.node_name()
                ));
                let result2 = node2019
                    .rpc_client()
                    .send_transaction_result(tx.data().into());
                assert!(
                    result2.is_err(),
