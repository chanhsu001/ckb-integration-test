...
pub struct RFC0028V2019;

impl Case for RFC0028V2019 {
    ...
    fn run(&self, nodes: Nodes) {
        ...
        ) in cases.into_iter().enumerate()
        {
            ...
            {
                let cloned_node2019 =
                    node2019.clone_node(&format!("{}-cloned-{}", node2019.node_name(), i));
                cloned_node2019.pull_node(&node).expect(&format!(
                    "{} pull {} should be ok",
                    cloned_node2019.node_name(),
                    node.node_name()
                ));
            }
        }
